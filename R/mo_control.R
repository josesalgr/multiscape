...
#' to infeasible runs, missing solution vectors, unexpected errors, and
#' AUGMECON slack-variable bounds.
#'
...
#'
#' \code{mo_control()} determines whether such events stop the entire
...
#' \code{\link{set_method_augmecon}} and has no effect on weighted-sum or
#' standard epsilon-constraint workflows.
#'
#' The value should be sufficiently large to avoid excluding valid solutions,
#' but unnecessarily large bounds can weaken the mixed-integer formulation and
#' reduce numerical performance. When possible, a problem-specific bound based
#' on the ranges of the constrained objectives should be used.
#'
...
#'   multi-objective methods. Defaults to \code{1e6}.
#'
#' @return An object of class \code{MOControl} containing the validated
#'   execution-control settings. The object is intended to be supplied to the
#'   \code{control} argument of a supported multi-objective method.
#'
#' @examples
